...
const displayDetails = (details) => {
  const itemsContainer = document.getElementById("items-container");
    ...
    <div class="items bg-slate-100 rounded container mx-auto mt-10 p-8">
     <h3 class="text-lg">${details.length ? details.length : "No"} items found for category</h3>
    </div>
    `;
  ...
  details.forEach((data) => {
    ...
    detailDiv.innerHTML = `
      ...
      src="${
        data.thumbnail_url ? data.thumbnail_url : "No image found"
      }" alt="">
    ...
    <div class="flex items-center justify-between pr-0 sm:pr-14">
     <div class="flex">
     <img class="object-cover w-12 h-12 rounded-full mr-2" src="${
       data.author.img
     }">
     <div>
